...
import { saleFinancials } from "./salesMetrics";
...
function SaleRow({ sale, onPaid, onEdit, editingDisabled }) {
  ...
  return <tr>
    ...
    <td data-label="Total"><strong>{money(financials.total)}</strong><details><summary>Rentabilidad</summary><dl className="admin-financial-details admin-financial-details--sale">
      <div><dt>Costo unitario</dt><dd>{financials.unitCost === null ? "Costo pendiente" : money(financials.unitCost)}</dd></div>
      <div><dt>Costo total</dt><dd>{costValue(financials.cost)}</dd></div>
      <div><dt>Ganancia bruta</dt><dd>{costValue(financials.grossProfit)}</dd></div>
    </dl>{sale.sale_status !== "sold" && <p className="admin-detail">Excluida de los totales por no ser una venta activa.</p>}</details></td>
...
export default function AdminSales({ sales, summary, loading, error, onRetry, onPaid, onRegistered, products, productsReady }) {
  const investments = useInvestmentRounds();
  ...
  return <>
    <section className="admin-summary" aria-label="Resumen de ventas">{[["total", "Total vendido", money(summary.total)], ["available", "Total pagado", money(summary.paid)], ["soldout", "Pendiente por cobrar", money(summary.pending)], ["disabled", "Unidades vendidas", summary.units], ["cost", "Costo de lo vendido", costValue(summary.cost)], ["profit", "Ganancia bruta", costValue(summary.grossProfit)], ["margin", "Margen bruto", costValue(summary.grossMargin, percentage)]].map(([key, label, value]) => <div key={key} className={`admin-stat admin-stat--${key}`}><span>{label}</span><strong>{loading || error ? "—" : value}</strong></div>)}</section>
    {!loading && !error && summary.missingCostCount > 0 && <p className="admin-registration-notice">Datos de costo incompletos: {summary.missingCostCount} venta(s) con costo pendiente. No se muestran costo total, ganancia ni margen hasta completar esos datos.</p>}
    <InvestmentDashboard rounds={investments.rounds} sales={sales} loading={loading || investments.loading} error={error || investments.error} onRetry={() => { investments.refresh(); if (error) onRetry(); }} />
